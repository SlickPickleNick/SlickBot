...
- Visible suggestions use the submitter profile image. Anonymous suggestions use the default Discord avatar thumbnail. Suggestion footers use SlickBot Suggestions branding instead of showing the submitter mention.
- Staff can change review status to Pending, Planned, Accepted, Denied, or Implemented and add staff responses/revision notes. Review status buttons open an optional details modal before updating. The active status button is green and inactive status buttons are gray. Status changes update only the original suggestion embed color. The separate revision-history embed stays muted gray, appears only after status/details changes, and is ordered oldest-to-newest.
...
- Added a tracked public Suggestions panel with post, edit, and refresh commands. The panel displays only the configured title, description, footer branding, and Submit Suggestion button, while keeping setup/channel details out of the public embed. The panel reposts to the bottom of the panel channel when new suggestions are submitted so the submit button remains visible. Added `/suggestion review-index` for a staff index linking to suggestion review messages, with Pending, Planned, Accepted, Denied, Implemented, and All filters.
...
- Counting supports a dedicated channel, starting/current number controls, record tracking, incorrect-entry resets, consecutive-turn prevention, accepted-message edit/delete protection, safe math expressions, invalid-message deletion, custom reset/milestone messages, milestone XP, configurable accepted/failed count reactions, ignored roles/users, staff correction/reset tools, and a contribution leaderboard.
...
- Application review index filter buttons now silently refresh the index without sending a separate hidden confirmation message.
