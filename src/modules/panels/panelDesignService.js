...
async function updatePanelDesign({ guildId, target, name = null, title = null, description = null, color = null }) {
  ...
  if (key === 'ticket' || key === 'tickets') {
    const result = await query(
      `INSERT INTO ticket_configs (guild_id, panel_title, panel_description, panel_color)
       VALUES ($1, $2, $3, $4)
  ...
  if (key === 'report' || key === 'reports') {
    const result = await query(
      `INSERT INTO report_configs (guild_id, panel_title, panel_description, panel_color)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (guild_id) DO UPDATE SET
  ...
  if (key === 'appeal' || key === 'appeals') {
    const result = await query(
      `INSERT INTO appeal_configs (guild_id, panel_title, panel_description, panel_color)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (guild_id) DO UPDATE SET
